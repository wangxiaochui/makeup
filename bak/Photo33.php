...
class Photo extends Base implements MakeupInterface
{
     ...
     */
    public function todo($image_list ,$pages=1, $w_h = 2, $is_use_temp=false, $temp_id=0)
    {
        ...
        else
        {
           ...
           // $edit_height =  ($height - $config['standard']['padding'][0]-$config['standard']['padding'][2]);
            ...
            if($allow == 0){ //方式一，对应$total_scale_x
                ...
                $top = $config['standard']['padding'][0] + $first_height+1;
...
//        var_dump($data);exit;
    ...
    private function commonTwoSecond($padding=[], $img_list = [], $flag = 1){
        ...
        $auto_pack[0]['top'] = $config['standard']['padding'][0];
        $auto_pack[0]['left'] = $config['standard']['padding'][3];
        $auto_pack[0]['height'] = $dis_height_f*$this->wh;

        if($sy < 0 ){  //
            $relative_height = 100;
            $relative_width = ($relative_height/$dis_height_f)*$edit_width;
...
//        }
        ...
        $auto_pack[1]['left'] = $config['standard']['padding'][3];
        $auto_pack[1]['height'] = $dis_height_s*$this->wh;

        if($sy < 0 ){  //
            $relative_height = 100;
            $relative_width = ($relative_height/$dis_height_s)*$edit_width;
            //var_dump($relative_width);exit;
            $top = 0;
            $left = ($relative_width - 100)/2;
            ...
            $relative_width = 100;
            $relative_height = ($img_height_f/$dis_height_s)*100;
            $left = 0;
            $top = ($relative_height - 100)/2;
        }
...
//        }
    ...
    private function commonOne($img_info, $padding=[]){
        ...
        $edit_height = (100 - $padding[0]-$padding[2]);
        ...
        if($img_scale > $edit_scale){ //横向切
            ...
            $relative_width = (($img_width/$edit_width)*100)/$this->wh;
            $relative_height = 100;
            ...
            $relative_height = (($img_height/$edit_height)*100)*$this->wh;
            $relative_width = 100;
            $relative_cut_left = 0;
